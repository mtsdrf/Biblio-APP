<template>
    <div>
                    ...
                    <div class="box-content" style="padding: 15px">
                        <div class="row" style="margin-bottom: 15px;">
                            ...
                            <div class="col-xs-6 col-sm-6 col-md-2 col-lg-2">
                                <router-link to='/prateleira-formulario'><button type="button" class="btn btn-primary waves-effect waves-light">Adicionar</button></router-link>
                            </div>
                        </div>
                                ...
                                <tbody id="table-sgbds">
                                    <tr v-for="prateleira in prateleiras" :key="prateleira.id">
                                        ...
                                        <td style="text-align: center">
                                            ...
                                            <button v-on:click="mostra_modal_excluir('modal-excluir', prateleira)" type="button" class="btn btn-danger waves-effect waves-light">
        ...
        <input type="hidden" id="id_prateleira_deletar">
        <modal name="modal-excluir" width="400px" height="200px" @before-open="set_id_prateleira">
            <div style="text-align:center">
                <h3>Confirmação...</h3>
            </div>
            <hr>
            <div style="margin-left: 15px">
                <p>Deseja excluir o prateleira?</p>
            </div>
            <hr>
...
<script>
    ...
    export default {
        ...
        methods: {
            editar: function(id) {
                this.$router.replace('/prateleira-formulario/' + id);
            },

            mostra_modal_excluir: function (modal_nome, prateleira){
                this.$modal.show(modal_nome, { prateleira });
            },
            ...
            set_id_prateleira: function(event) {
                document.querySelector("#id_prateleira_deletar").value = event.params.prateleira.id;
            },

            apagar: function () {
                this.isLoading = true;
                this.$modal.hide('modal-excluir');
                var id = document.querySelector("#id_prateleira_deletar").value;
                axios.delete("http://localhost:8000/api/prateleira/" + id, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + sessionStorage.getItem('token')
                    }
